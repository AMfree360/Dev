...
class BacktestEngine:
    ...
    def _process_entries(
                ...
                if df_entry is not None and len(df_entry) > 0:
                        ...
                        if len(self.positions) < max_positions:
                            ...
                            if len(entry_bars) > 0:
                                ...
                                # Validate stop
                                if (dir_int == 1 and stop_price < entry_price) or (dir_int == -1 and stop_price > entry_price):
                                        ...
                                        if len(entry_bars) > 0:
                                            ...
                                            if (dir_int == 1 and stop_price < entry_price) or (dir_int == -1 and stop_price > entry_price):
                                                ...
                                                if quantity > 0:
                                                    ...
                                                    try:
                                                        entry_result = self._enter_position(s, current_time, current_price, quantity)
                                                        if entry_result is True:
        ...
        for pending in self.pending_signals:
            ...
            if dir_int == 1 and stop_price >= entry_price:
                rejected_count['invalid_stop'] += 1
            ...
            signal_copy['entry_price'] = entry_price
            signal_copy['stop_price'] = stop_price
            signal = signal_copy
            
            # Enter and clear the queue (single position model)
            try:
                entry_result = self._enter_position(signal, current_time, current_price, quantity)
    ...
    def _calculate_position_size(
        ...
        if self.market_spec.asset_class == 'forex' and self.market_spec.contract_size is not None:
            ...
            if self.market_spec.asset_class == 'futures' and self.market_spec.contract_size is not None:
                ...
                # CRITICAL FIX: NT always trades at least 1 contract (Math.Max(1, quantity))
                # This means NT will take trades even if they exceed the risk amount
