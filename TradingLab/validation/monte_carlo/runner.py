...
class MonteCarloSuite:
    ...
    def run_conditional(
        ...
        # === Randomized Entry Test (Always Run) ===
        randomized_suit = test_suitability.get('randomized_entry')
        if show_progress:
            print("Running Randomized Entry Test (universal test)...")
    ...
    def _calculate_combined_score_conditional(
        ...
        # Calculate total weight from suitable tests that ran
        total_weight = sum(
            suit.priority 
            for test_name, suit in actual_suitability.items()
        ...
        if total_weight == 0:
            return {
                ...
                'bootstrap_score': 0.0,
                'randomized_entry_score': 0.0,
                'warnings': ['No suitable tests ran - cannot compute combined score'],
                'normalized_weights': {}
            }
        
        # Normalize weights
        normalized_weights = {
            test_name: suit.priority / total_weight
            for test_name, suit in actual_suitability.items()
            if test_name in actual_results
        }
        ...
        combined_score = (
            normalized_weights.get('permutation', 0.0) * perm_score +
            normalized_weights.get('bootstrap', 0.0) * bootstrap_score +
            normalized_weights.get('randomized_entry', 0.0) * randomized_score
        )
        
        # Calculate combined percentile (weighted average)
        combined_percentile = (
            normalized_weights.get('permutation', 0.0) * perm_percentile_avg +
            normalized_weights.get('bootstrap', 0.0) * bootstrap_percentile_avg +
            normalized_weights.get('randomized_entry', 0.0) * randomized_percentile_avg
        )
        
        # Calculate combined p-value (weighted average)
        combined_p_value = (
            normalized_weights.get('permutation', 0.0) * perm_p_value_avg +
            normalized_weights.get('bootstrap', 0.0) * bootstrap_p_value_avg +
            normalized_weights.get('randomized_entry', 0.0) * randomized_p_value_avg
        )
        ...
        return {
            ...
            'n_suitable_tests': len(actual_results)
        }
