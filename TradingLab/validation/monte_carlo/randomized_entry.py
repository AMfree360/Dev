...
class MonteCarloRandomizedEntry:
    ...
    def __init__(self, seed: int = 42, config: Optional[RandomizedEntryConfig] = None):
        """
    ...
    def extract_baseline_trades(
        ...
        for trade_idx, trade in enumerate(backtest_result.trades):
            ...
            if entry_bar_idx >= exit_bar_idx:
                raise RuntimeError(
                    f"Baseline trade {trade_idx}: entry_bar_index ({entry_bar_idx}) >= exit_bar_index ({exit_bar_idx})"
                )
    ...
    def randomize_entry(
        ...
        if mode == "price":
            ...
            elif hasattr(strategy, 'market_spec') and strategy.market_spec:
                # Try to get from market spec
                tick_size = getattr(strategy.market_spec, 'pip_value', tick_size) or tick_size
    ...
    def _calculate_take_profit(
        ...
        if direction == 'long':
            tp_price = entry_price + (tp_cfg.target_r * r_value)
        else:
            tp_price = entry_price - (tp_cfg.target_r * r_value)
    ...
    def simulate_trade_from_entry(
        ...
        if self.config.enabled and mode and mode != "auto":
            if mode == "timing":
                ...
                if hasattr(strategy, 'market_spec') and strategy.market_spec:
                    tick_size = getattr(strategy.market_spec, 'pip_value', tick_size) or tick_size
    ...
    def run(
        ...
        # Calculate observed metrics using SAME pipeline as real backtest
        observed_metrics = calculate_enhanced_metrics(backtest_result)
        
        # Add final_pnl if requested
        ...
        # Calculate baseline total PnL from individual trades (for comparison with random iterations)
