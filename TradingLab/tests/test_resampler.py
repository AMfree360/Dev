...
def test_parse_timeframe():
    ...
    assert parse_timeframe('1m') == '1T'
    assert parse_timeframe('15m') == '15T'
...
def test_resample_ohlcv_basic():
    ...
    assert df_1h.iloc[0]['open'] == df_1m.iloc[0]['open']  # First open
    assert df_1h.iloc[0]['close'] == df_1m.iloc[-1]['close']  # Last close
    assert df_1h.iloc[0]['high'] >= df_1m['high'].max() * 0.99  # Max high (approx)
    assert df_1h.iloc[0]['low'] <= df_1m['low'].min() * 1.01  # Min low (approx)
    assert df_1h.iloc[0]['volume'] == df_1m['volume'].sum()  # Sum volume
...
def test_resample_ohlcv_15m():
    ...
    # Should have 4 bars (60 minutes / 15 minutes)
    assert len(df_15m) == 4
