...
class TestEdgeLatency(unittest.TestCase):
    ...
    def test_weak_edge(self):
        ...
        r_multiples = np.array([0.1, 0.2, -0.1, 0.1, 0.0, 0.1, 0.2, -0.1, 0.1, 0.1])
        result = compute_edge_latency(r_multiples)
        
        self.assertIsNotNone(result['required_trades'])
        self.assertGreater(result['required_trades'], 100)  # Weak edge requires more trades
    ...
    def test_warning_high_latency(self):
        """Test that warning is issued for high latency."""
        # Very weak edge that will require > 300 trades
        r_multiples = np.random.normal(0.05, 1.0, 100)  # mean=0.05, std=1.0 → signal_ratio=0.05
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            result = compute_edge_latency(r_multiples)
            self.assertGreater(len(w), 0)
            self.assertTrue(any("very high" in str(warning.message).lower() for warning in w))
    
    def test_warning_low_signal_ratio(self):
        """Test that warning is issued for low signal ratio."""
        # Very low signal ratio
        r_multiples = np.random.normal(0.05, 1.0, 50)  # signal_ratio ≈ 0.05
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            result = compute_edge_latency(r_multiples)
            self.assertGreater(len(w), 0)
            self.assertTrue(any("signal ratio" in str(warning.message).lower() for warning in w))
