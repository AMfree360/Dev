...
**💡 Tip**: For a complete step-by-step workflow from development to validation, see the [Complete Workflow Guide](WORKFLOW.md).
...
```bash
python scripts/run_backtest.py --strategy titanea --data data/raw/BTCUSDT_1m.csv
```
...
1. **If results look good**: Proceed to [Validation](VALIDATION.md)
2. **If results need improvement**: Try [Optimization](OPTIMIZATION.md)
3. **To understand metrics better**: Read [Reports Guide](REPORTS.md)
