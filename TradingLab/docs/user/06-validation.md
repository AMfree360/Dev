...
**💡 Tip**: For a complete step-by-step workflow that includes validation, see the [Complete Workflow Guide](WORKFLOW.md). The workflow guide shows how validation fits into the overall strategy development process.
...
3. **Monte Carlo Suite** (Conditional):
   - **Permutation Test**: Tests if trade order matters (only if suitable)
   - **Block Bootstrap Test**: Tests robustness to market regime sequences (only if suitable)
   - **Randomized Entry Test**: Tests if strategy beats random entries (always suitable)
...
python3 scripts/optimize_strategy.py \
  ...
  --output strategies/titanea/config_optimized.yml
```
...
python3 scripts/run_training_validation.py \
  --strategy titanea \
  --data data/raw/EURUSD_2020-2022.csv \
  --config strategies/titanea/config_optimized.yml \
  --start-date 2020-01-01 \
  --end-date 2021-12-31 \
...
- ✅ Monte Carlo suite (only suitable tests):
  - Combined MC score ≥ 0.60
  - Combined MC percentile ≥ 70%
  - Adaptive pass requirement:
    - 3 suitable tests: at least 2 must pass
    - 2 suitable tests: at least 1 must pass (majority rule)
    - 1 suitable test: must pass
