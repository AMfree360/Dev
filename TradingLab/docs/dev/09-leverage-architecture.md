...
Located in `engine/market_spec.py`, this class encapsulates all market-specific rules:
...
Located in `engine/market_spec.py`, this class handles margin and position validation:
...
python3 scripts/run_backtest.py --strategy titanea --market EURUSD

# BTCUSDT with 1:1 leverage (spot)
python3 scripts/run_backtest.py --strategy titanea --market BTCUSDT

# ES futures with exchange-set margin
python3 scripts/run_backtest.py --strategy titanea --market ES
```
...
2. **Test leverage changes**:
   ```bash
   # With 30:1 leverage
   python3 scripts/run_backtest.py --strategy titanea --market EURUSD --leverage 30.0
   
   # With 1:1 leverage (no leverage)
   python3 scripts/run_backtest.py --strategy titanea --market EURUSD --leverage 1.0
   ```
