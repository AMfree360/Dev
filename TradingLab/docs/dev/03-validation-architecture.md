...
3. **Randomized Entry Test**: Tests if strategy edge exceeds random entries with same risk management
...
3. **Minimum Iterations**: All tests require N ≥ 1000 iterations
...
**Purpose**: Combine results from all three independent tests into a unified robustness assessment.
...
Per specification:
- **Randomized Entry**: 50% (most important - tests true edge)
- **Bootstrap**: 30% (tests regime robustness)
- **Permutation**: 20% (tests temporal skill)
...
- **Combined Score**: Weighted average of test scores (0-1)
- **Combined Percentile**: Weighted average of percentiles (0-100)
- **Combined P-value**: Weighted average of p-values (0-1)
...
3. **Monte Carlo Suite (Conditional)**
   - Combined MC score >= `min_mc_score` (only from suitable tests)
   - Combined MC percentile >= `min_mc_percentile` (only from suitable tests)
   - **Adaptive Pass Requirement**:
     - If 3 suitable tests: require at least 2 to pass
     - If 2 suitable tests: require at least 1 to pass (majority rule)
...
- Run only suitable Monte Carlo tests with N ≥ 1000
- Compute combined MC score from suitable tests only
