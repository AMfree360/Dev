...
❌ **Code Duplication**:
- Filter logic duplicated in each strategy (e.g., `TitanEAStrategy._check_regime_filters()`)
...
#### Market Profile (config/market_profiles.yml + engine/market_spec.py)
...
```python
from engine.market_spec import MarketSpec
...
1. **Market Profile (`config/market_profiles.yml` + `engine/market_spec.py`)**:
...
**Location**: `engine/market_spec.py` (add point_value and tick_value fields)
...
```yaml
# In strategies/titanea/config.yml
stop_loss:
  min_stop_distance:
    enabled: true
    type: "fixed"
...
- **Stepped**: R-based stepped trailing (like TitanEA)
