...
class FilterManager:
    ...
    def _build_calendar_filters(self, config: Dict) -> List[FilterBase]:
        ...
        # if calendar_cfg.get('day_of_week', {}).get('enabled', False):
        #     from strategies.filters.calendar import DayOfWeekFilter
        #     filters.append(DayOfWeekFilter(calendar_cfg['day_of_week']))
        ...
        if trading_sessions:
            ...
            # Pass the full calendar_cfg so TradingSessionFilter can access trading_sessions
            # Ensure trading_sessions is included in the config passed to the filter
            # ARCHITECTURE: Trading session filter should always be enabled if trading_sessions exist
            filter_config = calendar_cfg.copy()
            if 'trading_sessions' not in filter_config:
                filter_config['trading_sessions'] = trading_sessions
            # Explicitly enable the filter if trading_sessions exist
            filter_config['enabled'] = True
            session_filter = TradingSessionFilter(filter_config)
            import logging
            logger = logging.getLogger(__name__)
            logger.debug(f"Created TradingSessionFilter with {len(session_filter.sessions)} sessions, "
                        f"enabled sessions: {[name for name, sess in session_filter.sessions.items() if sess.enabled]}, "
                        f"filter enabled: {session_filter.is_enabled()}")
